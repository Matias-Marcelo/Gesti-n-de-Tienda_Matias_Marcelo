...
public class Shop {
	...
	// Constructor initializes the shop with an initial cash amount, inventory, and
	// sales array
	public Shop() {
		cash = 100.0;
		// sales = new Sale[10];
...
//                	shop.deleteProduct(); //Delete product of inventory
	...
	public void initSession() throws SQLException {
		...
		do {
			...
			// When the employee data is false, continue iterating. This is validated by
			// calling the login method with the employee object
		} while (!employee.login(numero, password));

		System.out.println("Login correcto");
		// Aquí puedes agregar cualquier otra lógica que quieras ejecutar después de
...
////        addProduct(new Product("Fresa", 5.00, true, 20, 10.00));
	...
	public void addStock(Product product, int stock) {
		...
		if (product != null) {
			...
			// System.out.println("The stock of the product " + name + " has been updated to
			// " + product.getStock());
		} else {
	...
	public void sale() {
		...
		// IF boolean test is false then declare variable deber (double) and give the
		// value of what the client owes.
		if (!test) {
			...
			try {
				...
				for (Sale sale : sales) {
					...
					StringBuilder line = new StringBuilder(
							counterSale + ";Client: " + sale.getClient() + "; Date=" + formattedData + "\n");
